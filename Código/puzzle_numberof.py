...
def a_estrela(puzzle, visualize=False):
    Node.reset_visited_nodes()
    start_node = Node(state=puzzle.start_state, cost=0, heuristic=calculate_heuristic_misplaced_tiles(puzzle.start_state, puzzle.goal_state))
    open_set = [start_node]
    closed_set = set()
    ...
    while open_set:
        ...
        for neighbor_state in puzzle.get_neighbors(current_node.state):
            if tuple(map(tuple, neighbor_state)) not in closed_set:
                neighbor_node = Node(state=neighbor_state, parent=current_node, cost=current_node.cost + 1, heuristic=calculate_heuristic_misplaced_tiles(neighbor_state, puzzle.goal_state))
                heapq.heappush(open_set, neighbor_node)

    return None, 0, Node.visited_nodes


def calculate_heuristic_misplaced_tiles(state, goal_state):
    misplaced_tiles = sum([1 for i, row in enumerate(state) for j, val in enumerate(row) if val != goal_state[i][j]])
